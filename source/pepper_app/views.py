...
class GetNewArticles(TemplateView):
    ...
    def post(self, request):
        ...
        if get_new_articles_form.is_valid():
            articles_to_retrieve = get_new_articles_form.cleaned_data[
            ...
            session_variables = {
                "get_new_articles_task_id": get_new_articles_task.id,
            ...
            context = {
                "get_new_articles_form": ScrapingRequest(
                    initial={"articles_to_retrieve": articles_to_retrieve}
                ),
                "get_new_articles_task_id": request.session.get(
                    "get_new_articles_task_id"
                ),
                "get_new_articles_result": request.session.get(
                    "get_new_articles_result"
                ),
                "get_new_articles_in_progress": request.session.get(
                    "get_new_articles_in_progress"
                ),
                "get_new_articles_finished": request.session.get(
                    "get_new_articles_finished"
                ),
            }
...
class CheckGetNewArticleTaskStatus(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        if task.ready():
            ...
            request.session["data"]["get_new_articles_in_progress"] = request.session[
                "get_new_articles_in_progress"
            ]
            request.session["data"]["get_new_articles_finished"] = request.session[
                "get_new_articles_finished"
            ]
            request.session["data"]["get_new_articles_result"] = request.session[
                "get_new_articles_result"
            ]
...
class CheckGetNewArticleTaskResult(TemplateView):
    ...
    def get(self, request):
        results = PepperArticle.objects.order_by("-date_added").order_by("-item_id")[
            : request.session.get("articles_to_retrieve")
        ][::-1]
...
class GetSearchedArticles(TemplateView):
    ...
    def post(self, request):
        ...
        if get_searched_articles_form.is_valid():
            articles_to_retrieve = get_searched_articles_form.cleaned_data[
                "articles_to_retrieve"
            ]
            searched_article = get_searched_articles_form.cleaned_data[
                "searched_article"
            ]
            ...
            if scrape_data == "Yes":
                get_searched_articles_task = scrape_searched_articles.delay(
                    searched_article, articles_to_retrieve
                )

                session_variables = {
                    "get_searched_articles_task_id": get_searched_articles_task.id,
            ...
            context = {
                "get_searched_articles_form": ScrapingSearchedArticleRequest(
                    initial={
                        "articles_to_retrieve": articles_to_retrieve,
                        "searched_article": searched_article,
                        "scrape_data": scrape_data,
                ...
                "get_searched_articles_task_id": request.session.get(
                    "get_searched_articles_task_id"
                ),
                "get_searched_articles_result": request.session.get(
                    "get_searched_articles_result"
                ),
                "get_searched_articles_in_progress": request.session.get(
                    "get_searched_articles_in_progress"
                ),
                "get_searched_articles_finished": request.session.get(
                    "get_searched_articles_finished"
                ),
...
class CheckGetSearchedArticleTaskStatus(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        if task.ready():
            ...
            request.session["searching_task_data"][
                "get_searched_articles_in_progress"
            ] = request.session["get_searched_articles_in_progress"]
            request.session["searching_task_data"][
                "get_searched_articles_finished"
            ] = request.session["get_searched_articles_finished"]
            request.session["searching_task_data"][
                "get_searched_articles_result"
            ] = request.session["get_searched_articles_result"]
...
class CheckGetSearchedArticleTaskResult(TemplateView):
    ...
    def get(self, request):
        ...
        results = PepperArticle.objects.filter(conditions).order_by("date_added")[
            : request.session.get("searched_articles_to_retrieve")
        ][::-1]

        session_variables = {
            "searched_articles_to_retrieve": False,
...
class ScrapeContinouslyTasks(TemplateView):
    ...
    def get(self, request):
        context = {
            "scrape_all_new_task_in_progress": request.session.get(
                "scrape_all_new_task_in_progress"
            ),
            "scrape_all_new_invalid_action": request.session.get(
                "scrape_all_new_invalid_action"
            ),
            "scrape_by_refreshing_task_in_progress": request.session.get(
                "scrape_by_refreshing_task_in_progress"
            ),
        }
    ...
    def post(self, request):
        ...
        scrape_by_refreshing_task_status = request.POST.get(
            "scrape_by_refreshing_task_status"
        )
        ...
        if scrape_all_new_task_status == "Start Scraping":
            if request.session.get("scrape_all_new_task_in_progress") == True:
                ...
                session_variables = {
                    "scrape_all_new_task_id": scrape_all_new_task.id,
        ...
        if scrape_all_new_task_status == "Stop Scraping":
            ...
            session_variables = {
                "scrape_all_new_task_in_progress": False,
        ...
        if scrape_by_refreshing_task_status == "Start Refreshing":
            if request.session.get("scrape_by_refreshing_task_in_progress") == True:
                ...
                session_variables = {
                    "scrape_by_refreshing_task_id": scrape_by_refreshing_task.id,
        ...
        if scrape_by_refreshing_task_status == "Stop Refreshing":
            scrape_by_refreshing_task_id = request.session.get(
                "scrape_by_refreshing_task_id"
            )
            ...
            session_variables = {
                "scrape_by_refreshing_task_in_progress": False,
    ...
    def post(self, request):
        ...
        if user_request_form.is_valid():
            ...
            try:
                ...
                logging.warning(
                    f"Saving data to csv failed: {e}\n Tracking: {traceback.format_exc()}"
                )

            context = {
                "user_request_form": UserRequestForm(
                    initial={
                        "desired_article": desired_article,
...
class ArticlePriceHistory(TemplateView):
    ...
    def generate_article_price_history_chart(self, filtered_data, article):
        ...
        discount_price_list = list(
            filtered_data.values_list("discount_price", flat=True)
        )
        ...
        chart_html = article_price_history_chart(
            discount_price_list, date_added_list, article_name_list, article
        ).to_html()
    ...
    def get(self, request):
        ...
        if article_price_history_form.is_valid():
            excluded_terms = self.request.GET.get("excluded_terms")
            conditions = self.searching_conditions(
                article, price_min, price_max, excluded_terms
            )
            filtered_data = PepperArticle.objects.filter(conditions).values(
                "discount_price", "date_added", "article_name"
            )

            chart_html = self.generate_article_price_history_chart(
                filtered_data, article
            )
