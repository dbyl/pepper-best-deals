...
from pepper_app.get_info import (
    GetItemAddedDate,
...
from pepper_app.populate_database import (
    LoadItemDetailsToDatabase,
...
class ScrapePage:
    def __init__(
        self,
        category_type: str,
        articles_to_retrieve: int,
        to_csv: bool = False,
        to_database: bool = True,
        to_statistics: bool = True,
        searched_article: str = "NA",
        scrape_continuously: bool = False,
        scrape_choosen_data: bool = True,
    ) -> None:
    ...
    def scrape_page(
        self, url_to_scrape: str, driver: webdriver = None
    ) -> BeautifulSoup:
    ...
    def accept_cookies_and_change_interval_if_searching(self, driver) -> None:
        ...
        try:
            ...
            driver.find_element(
                By.XPATH,
                "//button[contains(@class, 'overflow--wrap-on flex--grow-1 flex--fromW3-grow-0 width--fromW3-ctrl-m space--mb-3 space--fromW3-mb-0 space--fromW3-mr-2 button button--shape-circle button--type-primary button--mode-brand')]",
            ).click()
            select = Select(
                driver.find_element(
                    By.XPATH,
                    "//div[contains(@class, 'space--mt-3 space--t-3 space--r-3 space--l-3 border--t border--color-greyBackground')]//select[contains(@class, 'select-ctrl input clickable')]",
                )
            )
            select.select_by_value("0")
        except NoSuchElementException as e:
            logging.warning(
                f"Accepting cookies and changing time interval failed :\
    ...
    def infinite_scroll_handling(self) -> List[str]:
        ...
        try:
            ...
            while flag:
                ...
                flag_nowe = CheckConditions(
                    soup, self.start_page
                ).check_if_last_page_nowe()
                flag_search = CheckConditions(
                    soup, self.start_page
                ).check_if_last_page_search()

                if flag_nowe == False or flag_search == False:
                    flag = False

                if self.category_type == "search":
                    flag_found = CheckConditions(
                        soup, self.start_page
                    ).check_if_no_items_found()
                    if flag_found == False:
                        all_items = list()
                        return all_items
    ...
    def get_items_details_depending_on_the_function(self):
        ...
        if self.scrape_continuously == True and self.scrape_choosen_data == False:
            ...
            raise Exception(
                f"Matching get_items_details depending on the selected \
    ...
    def get_items_details(self, prepared_articles):
        ...
        if self.to_statistics:
            try:
                ...
                logging.warning(
                    f"Populating ScrapingStatistics table failed: {e}\n Tracking: {traceback.format_exc()}"
                )
        return all_items
    ...
    def get_scraping_stats_info(
        self, action_execution_datetime: datetime
        ...
        stats = [
            category_type,
            retrieved_articles_quantity,
            time_of_the_action,
            action_execution_datetime,
            searched_article,
            to_csv,
            to_database,
            scrape_continuously,
...
class CheckConditions:
    ...
    def check_if_last_page_nowe(self) -> bool:
        ...
        try:
            ...
            raise Exception(
                f"Checking if page in nowe category is last failed:\
                            {e}\n Tracking: {traceback.format_exc()}"
            )
    ...
    def check_if_last_page_search(self) -> bool:
        ...
        try:
            searched_ending_string = self.soup.find_all(
                "h3", {"class": "size--all-xl text--b"}
            )[0].get_text()
            searched_articles_number = self.soup.find_all(
                "span", {"class": "text--color-charcoalTint size--all-m"}
            )[0].get_text()
            searched_articles_number = int(
                searched_articles_number.replace(" ", "").strip("\n\t okazje")
            )
            if (
                searched_ending_string.startswith("Ups")
                and searched_articles_number > 0
            ):
