...
from pepper_app.forms import PassChangeForm, PassResetForm, PassSetForm
from pepper_app.views import (
    ArticlePriceHistory,
...
urlpatterns = [
    ...
    path(
        "get_new_articles/",
        GetNewArticles.as_view(template_name="features/get_new_articles.html"),
        name="get_new_articles",
    ),
    path(
        "get_new_articles_check/<get_new_articles_task_id>/",
        CheckGetNewArticleTaskStatus.as_view(),
        name="get_new_articles_check",
    ),
    path(
        "get_new_articles_result/",
        CheckGetNewArticleTaskResult.as_view(),
        name="get_new_articles_result",
    ),
    path(
        "get_searched_articles/",
        GetSearchedArticles.as_view(
            template_name="features/get_searched_articles.html"
        ),
        name="get_searched_articles",
    ),
    path(
        "get_searched_articles_check/<get_searched_articles_task_id>/",
        CheckGetSearchedArticleTaskStatus.as_view(),
        name="get_searched_articles_check",
    ),
    path(
        "get_searched_articles_result/",
        CheckGetSearchedArticleTaskResult.as_view(),
        name="get_searched_articles_result",
    ),
    path("scrape/", ScrapeContinouslyTasks.as_view(), name="scrape"),
    path("requests/", PriceAlertRequest.as_view(), name="requests"),
    path(
        "price_history_chart/",
        ArticlePriceHistory.as_view(),
